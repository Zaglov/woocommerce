...
class CustomOrdersTableController {
	 ...
	 */
	final public function init( OrdersTableDataStore $data_store, DataSynchronizer $data_synchronizer, BatchProcessingController $batch_processing_controller, OrdersTableRefundDataStore $refund_data_store ) {
	...
	private function process_pre_update_option( $option, $old_value, $value ) {
		...
		/*
		 * Disable for now to aid with testing.
		 * $sync_is_pending = $this->data_synchronizer->get_current_orders_pending_sync_count() !== 0;
		 * if ( $sync_is_pending ) {
